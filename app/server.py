...
def _api_bearer_matches_cached_token(request: Request) -> bool:
...
def _authorized_browser_or_api(request: Request) -> bool:
    """Session cookie (browser) or bearer token (CLI / API clients)."""
    return _restore_session_if_token_valid(request) or _api_bearer_matches_cached_token(
        request
    )
        ...
        try:
            ...
            if payload_to_store is None:
                if holdings_payload.get("error"):
                    ...
                    try:
                        fut = pool.submit(_build_mf_underlying_breakdown, mf_holdings)
...
async def _build_dashboard_view_model(
    request: Request,
    *,
    ...
    timings: list[tuple[str, float]] = []
    authorized = (
        _authorized_browser_or_api(request)
        if allow_bearer
        else _restore_session_if_token_valid(request)
    )
    if not authorized:
...
async def dashboard(request: Request):
    ...
    context, redirect = await _build_dashboard_view_model(request, allow_bearer=False)
...
async def api_v1_portfolio_snapshot(request: Request) -> JSONResponse:
    ...
    context, redirect = await _build_dashboard_view_model(request, allow_bearer=True)
    if redirect is not None:
...
async def dashboard_mf_underlyings(
    ...
    if not _authorized_browser_or_api(request):
        return JSONResponse({"error": "unauthorized"}, status_code=401)
...
def main() -> None:
    ...
    socket is listening (only for ``python -m app.server``, not ``uvicorn`` CLI).
    """
    import uvicorn
